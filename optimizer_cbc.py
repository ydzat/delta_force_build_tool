...
class Optimizer:
    def __init__(self, data_manager, weapon_type, weapon_name):
        ...
        self.model = LpProblem(name="Weapon_Optimization", sense=LpMaximize)
        self.choices = {}
    ...
    def setup_model(self):
        ...
        for part, items in parts.items():
            ...
            for item, effects in items.items():
                self.choices[part][item] = LpVariable(f"{part}_{item}", cat="Binary")

        # Each part can have at most one attachment selected
        for part, items in parts.items():
            self.model += lpSum(self.choices[part][item] for item in items) <= 1, f"choose_one_{part}"
                    ...
                    for item in self.weapon_data["parts"][slot].keys():
                        ...
                        if total_dependencies > 1:
                            ...
                            self.model += (
                                ...
                                f"bigM_dependency_{part}_{candidate}_enables_{slot}_{item}"
                            )
                        else:
                            # Direct constraints for single dependencies
                            self.model += (
                                slot_var <= candidate_var,
                                f"direct_dependency_{part}_{candidate}_controls_{slot}_{item}"
                            )
                ...
                for slot in conflict_slots:
                    for item in self.weapon_data["parts"][slot].keys():
                        ...
                        self.model += (
                            ...
                            f"bigM_conflict_{part}_{candidate}_disables_{slot}_{item}"
                        )
    ...
    def get_results(self):
        ...
        """
        if value(self.model.objective) is None:
            return {"status": "No optimal solution found."}

        selected_parts = {
            part: [item for item, var in items.items() if value(var) > 0.5]
            for part, items in self.choices.items()
        }
        final_attributes = {key: value(self.total_attributes[key]) for key in self.attribute_keys}
            ...
            if attribute in self.choices:
                for item in self.choices[attribute]:
                    if item == value:
                        ...
                        if operator == "=":
                            self.model += var == 1, f"constraint_{attribute}_{item}_eq_1"
                        elif operator == "<=":
                            self.model += var <= 1, f"constraint_{attribute}_{item}_lte_1"
                        elif operator == ">=":
                            self.model += var >= 1, f"constraint_{attribute}_{item}_gte_1"
            elif attribute in self.total_attributes:
                attr_value = self.total_attributes[attribute]
                if operator == "=":
                    self.model += attr_value == value, f"constraint_{attribute}_eq_{value}"
                elif operator == "<=":
                    self.model += attr_value <= value, f"constraint_{attribute}_lte_{value}"
                elif operator == ">=":
                    self.model += attr_value >= value, f"constraint_{attribute}_gte_{value}"
            else:
